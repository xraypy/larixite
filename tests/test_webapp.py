...
class TestOutput:
    ...
    def test_feff_output_zn_k(self, client):
        self._setup(client)
        rv = client.get("/output/test.cif/Zn/1/K/7.0/0/feff/feff.inp")
        assert rv.status_code == 200
        data = rv.get_data(as_text=True)
    ...
    def test_feff_output_with_h(self, client):
        self._setup(client)
        rv = client.get("/output/test.cif/Zn/1/K/7.0/1/feff/feff.inp")
        assert rv.status_code == 200

    def test_fdmnes_output(self, client):
        self._setup(client)
        rv = client.get("/output/test.cif/Zn/1/K/7.0/0/fdmnes/fdmfile.txt")
        assert rv.status_code == 200
        data = rv.get_data(as_text=True)
    ...
    def test_fdmnes_output_o_edge(self, client):
        self._setup(client)
        rv = client.get("/output/test.cif/O/1/K/7.0/0/fdmnes/fdmfile.txt")
        assert rv.status_code == 200
...
class TestZipfile:
    def test_feff_zip(self, client):
        _upload(client, ZNO_TEXT)
        rv = client.get("/zipfile/test.cif/Zn/K/7.0/0/feff/test_zoom")
        assert rv.status_code in (200, 500)

    def test_fdmnes_zip(self, client):
        _upload(client, ZNO_TEXT)
        rv = client.get("/zipfile/test.cif/Zn/K/7.0/0/fdmnes/test_zoom")
        assert rv.status_code in (200, 500)
